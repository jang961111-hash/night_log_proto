...
export function SettingsScreen({
  ...
  const canSave = name.trim().length > 0 && job.trim().length > 0 && !loading;
  ...
  const confirmResetData = () => {
    Alert.alert("내 기록 초기화", "이 계정의 일기 기록, 결과, 일정 초안을 모두 삭제합니다.", [
  ...
  const confirmDeleteAccount = () => {
    Alert.alert("계정 삭제", "계정과 관련된 로컬 데이터가 모두 삭제됩니다. 이 작업은 되돌릴 수 없습니다.", [
      { text: "취소", style: "cancel" },
      {
        text: "계정 삭제",
        style: "destructive",
  ...
  return (
    <SafeAreaView style={styles.root}>
      ...
      >
        <ScreenFadeIn>
          <AppHeader title="설정" subtitle="내 정보, 개인화, 데이터 정책을 관리합니다" onBack={onBack} />
        </ScreenFadeIn>
        ...
        <ScreenFadeIn delay={170}>
          <View style={styles.card}>
            ...
            <AppButton label="관심사 다시 선택" onPress={onOpenInterests} variant="outline" disabled={loading} />
        ...
        <ScreenFadeIn delay={220}>
          <View style={styles.card}>
            ...
            <AppButton label="내 기록 초기화" onPress={confirmResetData} variant="outline" disabled={loading} />
            <AppButton label="로그아웃" onPress={onLogout} variant="outline" disabled={loading} />
            <AppButton label="계정 삭제" onPress={confirmDeleteAccount} variant="ghost" disabled={loading} />
          </View>
        </ScreenFadeIn>
      </ScrollView>
    </SafeAreaView>
